...
class DXslider {
  ...
  init() {
    ...
    this.preButton.addEventListener(
      "click",
      function (e) {
        if (!this.animating) {
          this.left = false;
  ...
  settingStyle() {
    ...
    this.images.style.height = this.canvasBox.style.height =
      (this.imagesWidth * this.height) / this.width + "px";
  ...
  update(startTime, change, duration, easingFunc) {
    ...
    if (time < duration) {
      ...
      requestAnimationFrame(
        this.update.bind(this, startTime, change, duration, easingFunc)
      );
    } else {
      if (this.left) {
  ...
  render(progress, position) {
    for (var i = 0, len = this.canvasArray.length; i < len; i++) {
      ...
      canvas.style.setProperty(
        "-webkit-transform",
        "translate(" +
          (progress * position - (len / 2 - i) * this.imagesWidth) +
          "px, 0)"
      );
      ...
      context.drawImage(
        this.canvasArray[i].lighten,
        ((len / 2 - i) * this.imagesWidth - progress * position) *
          this.dpi *
          this.paraEffect,
        0,
        this.width,
